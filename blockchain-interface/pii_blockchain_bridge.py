...
class PIIBlockchainBridge:
    ...
    def __init__(self, bridge_port: int = 5002):
        ...
        # Configuration
        self.stacks_node_url = os.getenv(
    ...
    def init_local_db(self):
        ...
        with sqlite3.connect('pii_blockchain_bridge.db') as conn:
            cursor = conn.cursor()
        ...
        def get_blockchain_status(document_hash):
            ...
            try:
                ...
                return jsonify({
                    ...
                    'blockchain_confirmed': sum(1 for tx in transactions if tx[8] == 'confirmed')
                })
    ...
    def process_pii_data(self, filename: str, extracted_text: str, sensitive_data: Dict) -> Dict:
        ...
        for entity_text, pii_info in sensitive_data.items():
            ...
            # Store a one-way hash of the PII text — never the raw value
            entity_text_hash = hashlib.sha256(entity_text.encode()).hexdigest()
            ...
            """, (
                document_hash, safe_token, entity_type, confidence_score,
                is_sensitive, entity_text_hash, json.dumps({
                    'filename': filename,
                    'processing_timestamp': datetime.now().isoformat()
                })
            ))

            # Simulate hackathon confirmation
            cursor.execute("""
    ...
    def start_bridge_server(self):
        ...
        self.app.run(host='0.0.0.0', port=self.bridge_port,
...
def enhanced_process_pdf_with_blockchain(pdf_path, output_pdf, filename, blur=False):
    ...
    blockchain_result = interceptor.send_pii_to_blockchain(
        filename, extracted_text, sensitive_data)
...
def enhanced_process_image_with_blockchain(image_path, output_image_path, filename, blur=False):
    ...
    # Process normally
    output_path, extracted_text = process_image(
        image_path, output_image_path, blur)
    sensitive_data = get_sensitive_data(extracted_text)
